...
function EditPost() {
  ...
  useEffect(() => {
    if (state.sendCount) {
      ...
      async function savePost() {
        try {
          const response = await Axios.post(`/posts/${state.id}/edit`, { title: state.title.value, body: state.body.value, token: appState.user.token }, { cancelToken: ourRequest.token });
  ...
  useEffect(() => {
    ...
    async function fetchPost() {
      try {
        const response = await Axios.get(`/posts/${state.id}`, { cancelToken: ourRequest.token });
        if (response.data) {
          dispatch({ type: "fetchComplete", value: response.data });
          if (appState.user.username != response.data.author.username) {
            appDispatch({ tyoe: "flashMessage", value: "You do not have permission to edit that post" });
            navigate("/");
          }
        } else {
