...
function Search() {
  ...
  useEffect(() => {
    ...
    // Clean up after component is not used
  ...
  useEffect(() => {
    if (state.searchTerm.trim()) {
      ...
      // Cleanup function will clear timeout so our server isn't flooded with requests
            ...
            {Boolean(state.results.length) && (
              <div className="list-group shadow-sm">
                <div className="list-group-item active">
                  <strong>Search Results</strong> ({state.results.length} {state.results.lenght > 1 ? "items" : "item"} found)
                </div>
